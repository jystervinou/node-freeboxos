...
class Freebox extends EventEmitter {
	 ...
	 * @param {Object}
	 *            [configuration.app] - Application configurations
	 */
	constructor(configuration) {
		...
		if (configuration.jsonPath) {
			...
			debug("jsonContent=", jcontent);
			var content = JSON.parse(jcontent);
			debug("constructor", "JSON=", content);
			Object.assign(app, content);

			this.on('app', (app)=> {
				var jcontent = JSON.stringify(app);

				debug("constructor:onApp", "Save jcontent=", jcontent);
	 ...
	 * @param {function}
	 *            [callback] - Callback which was called for result
	 * @returns Promise
	 */
	...
	_waitApplicationGranted(delay, callback) {
		...
		var retryCb = (error) => {
			...
			setImmediate(() => this._waitStatusGranted(delay, callback));
		};
...
// error.response=body.result;
	 ...
	 * @param {string}
	 *            action - The action to do if and id is submited. Could be read, log, udpate, delete and deleteAndErase -
	 *            delete the download and erase the files downloaded. If null, it's set to read.
	 * @param {json}
	 *            [params] - If action update, the item to update.
	 * @param {function}
	 *            [callback] - Callback which was called for result
	 * @returns {Promise|undefined}
	 */
	 ...
	 * @param {string}
	 *            [dir] - The download destination directory
	 * @param {bool}
	 ...
	 *            [password] - Password
	 * @param {string}
	 *            [archive_password] - Password to decompress the archive if encrypted.
	 * @param {function}
	 *            [callback] - Callback which was called for result
	 * @returns {Promise|undefined}
	 */
	 ...
	 *            id - Call id
	 * @param {string}
	 *            [action=Read] - The action to do. Read (default), update, delete.
	 * @param {json}
